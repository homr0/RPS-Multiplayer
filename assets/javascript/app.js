$(document).ready(function() {
    ...
    // Clears the gameplay.
    function finishGame(play1, play2) {
        ...
        if(play1 == play2) {
            ...
            // Goes through the chat log for the autoGM and removes them.
            setTimeout(function() {
                $(".chat.logGM").remove();

                database.ref("chat").orderByChild("uid").equalTo("autoGM").once("value").then(function(snapshot) {
                    let dialog = snapshot.val();
                    $(Object.keys(dialog)).each(function(index, logGM) {
                        database.ref("chat/" + logGM).remove();
                    });
                });
            }, 1000);
        }
    ...
    chatRef.on("child_added", function(chats) {
        ...
        $(chats.val()).each(function(index, value) {
            var chat = $("<li>").addClass("chat").attr({
                "data-uid": value.uid
            });
